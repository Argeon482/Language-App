...
export default function App() {
  ...
  const lastHeaderStateChange = useRef<number>(0);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      const now = Date.now();
      // Ignore scroll events during a 350ms lockout period after a state change.
      // This completely prevents jitter/bouncing loops triggered by layout/height shifts.
      if (now - lastHeaderStateChange.current < 350) {
      ...
      if (delta > 0 && currentScrollY > 30) {
        // Hide immediately on any scroll down past 30px
        setShowHeaderDetails(prev => {
          if (prev) {
            lastHeaderStateChange.current = now;
            return false;
          }
          return prev;
        });
    ...
    const handleTouchStart = (e: TouchEvent) => {
      ...
      startedAtTop = window.scrollY <= 5;
    };

    const handleTouchMove = (e: TouchEvent) => {
      const now = Date.now();
      if (now - lastHeaderStateChange.current < 350) return;

      if (startedAtTop && window.scrollY <= 5) {
        const touchCurrentY = e.touches[0].clientY;
        const diffY = touchCurrentY - touchStartY; // positive diffY means dragging down / swipe down
        
        if (diffY > 60) { // Deliberate swipe down at the top of the page
          setShowHeaderDetails(prev => {
            if (!prev) {
              lastHeaderStateChange.current = now;
              return true;
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      if (window.scrollY <= 2 && e.deltaY < -25) { // Trackpad or mouse wheel scroll up (swipe down gesture) when already fully at the top
        setShowHeaderDetails(prev => {
          if (!prev) {
            lastHeaderStateChange.current = now;
            return true;
  ...
  const handleFetchTranscript = async () => {
    if (!promptYoutubeId.trim()) {
      setTranscriptFetchError('Please provide a YouTube Link or ID first.');
      return;
    }
    ...
    try {
      ...
      if (!data.transcript || data.transcript.length === 0) {
        throw new Error('No transcripts returned for this video.');
      }
      
      const formattedLines = data.transcript.map((item: any) => {
        let totalSeconds = Math.floor(item.offset || 0);
        if (item.offset && item.offset > 5000) {
          const isDecimal = item.offset.toString().includes('.');
          if (!isDecimal) {
            totalSeconds = Math.floor(item.offset / 1000);
          }
        }
      ...
      setPromptTranscript(formattedLines);
      setSelectedPromptType('transcript');
      if (data.videoId) {
        setPromptYoutubeId(data.videoId);
      }
    } catch (err: any) {
      console.error(err);
      setTranscriptFetchError(err.message || 'Error loading transcript. Ensure video is public and has captions.');
    } finally {
                        ...
                        <div>
                          ...
                          <div className="flex gap-1.5">
                            <input
                              ...
                              className="flex-1 bg-slate-950 border border-slate-800 rounded-lg px-2.5 py-1 text-[11px] text-slate-200 focus:border-indigo-500 outline-none placeholder:text-slate-600"
                            />
                            <button
                              type="button"
                              onClick={handleFetchTranscript}
                              disabled={isFetchingTranscript}
                              className={`px-3 py-1.5 text-[10px] font-bold rounded-lg transition-all flex items-center gap-1.5 shrink-0 ${
                                isFetchingTranscript
                                  ? 'bg-slate-800 text-slate-500 cursor-not-allowed border border-slate-700'
                                  : 'bg-indigo-600 hover:bg-indigo-500 text-white border border-indigo-500/20 shadow-lg shadow-indigo-600/10'
                              }`}
                              title="Fetch subtitles/transcript directly from YouTube"
                            >
                              {isFetchingTranscript ? (
                                <>
                        ...
                        <div className="text-center py-2 space-y-4 flex-1 flex flex-col justify-center">
                          ...
                          <div className="bg-slate-950/80 px-4 py-2.5 rounded-xl text-slate-400 italic max-w-md w-full mx-auto border border-slate-850 text-left">
                            <span className="block text-[9px] uppercase font-bold text-slate-500 tracking-widest mb-0.5">
                              {studyRole === 'spanish-learner' ? 'Literal Word-for-Word Equivalent' : 'Word-for-Word Mapping'}
                            </span>
                            <span className={`text-slate-300 block leading-relaxed ${
                              ...
                              {studyRole === 'spanish-learner' 
                                ...
                                : activePhrase.breakdown && activePhrase.breakdown.length > 0
                                  ? activePhrase.breakdown.map(item => `${item.meaning} (${item.word})`).join(' • ')
                                  : `"${activePhrase.literal}"`
                              }
                            </span>
                          </div>
                      ...
                      >
                        ...
                        <p className="text-slate-300 leading-relaxed">
                          {studyRole === 'spanish-learner' ? (
                            ...
                            <>
                              {uiLang === 'es' ? 'La correspondencia palabra por palabra es:' : 'The word-for-word mapping is:'} <br />
                              <span className="italic text-indigo-300">
                                {quizQuestion.breakdown && quizQuestion.breakdown.length > 0
                                  ? quizQuestion.breakdown.map(item => `${item.meaning} (${item.word})`).join(' • ')
                                  : `"${quizQuestion.literal}"`
                                }
                              </span>
                            </>
                          )}
